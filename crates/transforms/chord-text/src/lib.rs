//! chord-text — text-generation / chat plug-in (`text -> text`).
//!
...
//! - `model` — GGUF path (default `$CHORD_TEXT_MODEL` or the Qwen3-30B-A3B GGUF under the XDG models dir)
...
impl Unary for Text {
    ...
    fn describe(&self) -> &str {
        "text generation / chat (llama.cpp)"
    }
    fn backend(&self) -> &str {
        "llama.cpp"
    }
...
/// Resolve the GGUF model path: `model` option, else `$CHORD_TEXT_MODEL`, else
...
fn resolve_model(opts: &Options) -> Result<PathBuf> {
    ...
    if let Ok(m) = std::env::var("CHORD_TEXT_MODEL") {
        if !m.is_empty() {
            return chord_hf::resolve(&m);
        }
    }
    ...
    Err(ChordError::ModelMissing {
        what: "text model".to_string(),
        hint: "run `chord pull text`, or set --model to a GGUF path (or $CHORD_TEXT_MODEL)"
            .to_string(),
    }
    .into())
}
